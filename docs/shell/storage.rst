...
pruned while the chain is growing. In *Full* mode, every block that are
...
``head.last_allow_fork_level``, that contain all the blocks of a
...
- The *caboose* which represent the lowest block known by the
...
The *checkpoint* is also a special value which indicates the block
...
The context is a versioned key/value store that associate for each
...
resulting context of valid blocks application are committed on disk.
