...
History modes uses some markers which are used to describe the state
of the storage:
...
reconstruct`` on your tezos node. Note that the storage reconstruction
is a long process that, on the main network, may requires days to
complete.
